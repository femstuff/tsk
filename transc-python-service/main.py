...
# ========== ВАЖНО: Добавляем FFmpeg в PATH ==========
# Укажите правильный путь к FFmpeg на вашем компьютере
FFMPEG_PATH = r"C:\Users\Руслан\Downloads\ffmpeg-8.0.1-essentials_build\ffmpeg-8.0.1-essentials_build\bin"
...
try:
    model = whisper.load_model("small")  # Для русского языка лучше small или medium
...
async def transcribe(file: UploadFile = File(...)):
    ...
    # Сохраняем во временный файл с явным расширением
    ...
    try:
        # Транскрипция с дополнительной диагностикой
        print("🔊 Начинаем транскрипцию...")
        
        # Сначала проверим файл через ffprobe
        try:
        ...
        print(f"⏱️  Длительность аудио: {result.get('duration', 0):.2f} сек")
        ...
        if os.path.exists(tmp_path):
            os.remove(tmp_path)
            print(f"🗑️  Временный файл удален: {tmp_path}")
